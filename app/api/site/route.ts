...
export async function GET() {
  try {
    ...
    const [reviews, articles, gallery, services, faq, settings] = await Promise.all([
      ...
      db.prepare("SELECT id, title, slug, meta_description, excerpt_ar, excerpt_en, cover_image, body, conclusion, category, author, featured, faq_items, status, created_at, updated_at, publish_at FROM articles WHERE status = ? AND (publish_at IS NULL OR publish_at = '' OR publish_at <= ?) ORDER BY featured DESC, updated_at DESC, id DESC LIMIT 8").bind("published", new Date().toISOString()).all(),
